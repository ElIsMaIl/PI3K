---
title: "UMAP visualisation from cell cycle phases in combination with the different sample tags"
description: |
...
DimPlot(integrated_seurat, group.by = "Phase") +
  labs(title = "UMAP showing all samples \n grouped by cell cylce phases.")
```
...
DimPlot(integrated_seurat,
        group.by = "Phase",
        split.by = "Sample_Tag") +
  labs(title = "UMAP of all samples grouped by their sample tags \n and splitted by cell cycle phases")
```
...
# Extract identity and cell cylce phases from seurat object to determine the number of cells per cluster per cell cycle phase
n_cells_cycle <- FetchData(integrated_seurat,
                           vars = c("ident", "Phase")) %>%
...
datatable(n_cells_cycle,
          caption = "The table shows the number of cells per cluster per cell cycle phase.")
```
