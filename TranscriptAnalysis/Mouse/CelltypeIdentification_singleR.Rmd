...
seurat_integrated_filtered <- readRDS("E://CCCC/PI3K/data/SeuratObjects/demultiplexed/Mouse/seurat_integrated_filtered_mouse.rds")
```
...
results_main <- SingleR(test = as.SingleCellExperiment(seurat_integrated_filtered), ref = ref, labels = ref$label.main)
```

```{r adding-singleR-labels-to-seuratObject}
seurat_integrated_filtered@meta.data$singlr_labels <- results_main$labels
```
...
```{r}
metadata <- seurat_integrated_filtered@meta.data
```
...
```{r}
saveRDS(seurat_integrated_filtered_renamedidents, "E://CCCC/PI3K/data/SeuratObjects/cellfiltering_umi/Mouse/SingleR/seurat_integrated_singleR.rds")
```
