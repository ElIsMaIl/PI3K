...
knitr::kable(all.mito,
             caption = "This table shows the unique number of mt_genes from all samples. In total 214 mt_genes were found in all six samples")
```
...
```{r goplot}
goplot(egohuman_mt) + labs(caption = "Goplot shows the different relations derived from the go enrichtment results.")
```

```{r dotplot}
dotplot(egohuman_mt) + labs(caption = "Dotplot shows the different gene ratios, counts and the adjusted p.values from the go enrichtment results.")
```