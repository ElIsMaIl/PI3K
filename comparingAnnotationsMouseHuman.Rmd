...
- [ ] Run pipeline for 7273 [läuft] Maus
...
```{r}
load("data/unfilteredSeurat_1820.Rdata")
...
```{r}
load("data/unfilteredSeurat_1822.Rdata")
...
```{r}
#| fig.keep = "all",
#| fig.cap = "The first Venn-diagram shows the number of cells from the samples 1820 and their overlap. While the second Venn-Diagram shows the overlap between the human and mouse sample from 1820. The genes which are overlapping are shown in the last output."

#---- extract cell list from human and mouse from their Seurat object
cell_List_human <- Cells(list_seurat_1820h[["1820"]]@assays[["RNA"]]@data)
cell_List_mouse <- Cells(list_seurat_1820m[["1820"]]@assays[["RNA"]]@data)
...
gene_List_mouse <- row.names(list_seurat_1820m[["1820"]]@assays[["RNA"]]@data)


#---- plot Venn diagrams
venn_cells1820 <- venn(list(Human = cell_List_human,
                            Mouse = cell_List_mouse))

venn_genes1820 <- venn(list(Human = gene_List_human,
                            Mouse = gene_List_mouse))

#---- show intersection genes between both samples

isect1820 <- attr(venn_genes1820, "intersections")
isect1820$"Human:Mouse"

```

```{r}
#| fig.keep = "all",
#| fig.cap = "The first Venn-diagram shows the number of cells from the samples 1822 and their overlap. While the second Venn-Diagram shows the gene overlap between the human and mouse sample from 1822. The genes which are overlapping are shown in the last output."

#---- extract cell list from human and mouse from their Seurat object
cell_List_h <- Cells(list_seurat_1822h[["1822"]]@assays[["RNA"]]@data)
cell_List_m <- Cells(list_seurat_1822m[["1822"]]@assays[["RNA"]]@data)

#---- extract gene list from human and mouse from their Seurat object
gene_List_h <- row.names(list_seurat_1822h[["1822"]]@assays[["RNA"]]@data)
gene_List_m <- row.names(list_seurat_1822m[["1822"]]@assays[["RNA"]]@data)


#---- plot Venn diagrams
venn_cells1822 <- venn(list(Human = cell_List_h,
                            Mouse = cell_List_m))

venn_genes1822 <- venn(list(Human = gene_List_h,
                            Mouse = gene_List_m))

#---- show intersection genes between both samples

isect1822 <- attr(venn_genes1822, "intersections")
isect1822$"Human:Mouse"
