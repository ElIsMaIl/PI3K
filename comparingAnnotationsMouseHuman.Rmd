...
```{r load-data_1820human, eval=FALSE}
...
lapply(list_seurat, function(x) dim(x) %>% enframe(name = "dim")) %>% 
  ...
  mutate(filter = "none") %>% datatable()

mypal_1820human <- pal_jama()(2)
```

```{r load-data_1822human}
...
mypal_1822human <- pal_jama()(2)
```

```{r load-data_1820mouse}
...
mypal_1820mouse <- pal_jama()(2)
```

```{r load-data_1822mouse}
...
```{r load-data_1820_human}
load("data/unfilteredSeurat_1820_human.Rdata")
lapply(list_seurat, function(x) dim(x) %>% enframe(name = "dim")) %>% 
  bind_rows(.id = "experiment") %>% 
  mutate(dim = ifelse(dim == 1, "nrGenes", "nrCells")) %>% 
  datatable()  
mypal_1820h <- pal_jama()(2)
```

```{r load-data_1822_human}
load("data/unfilteredSeurat_1822_human.Rdata")
lapply(list_seurat, function(x) dim(x) %>% enframe(name = "dim")) %>% 
  bind_rows(.id = "experiment") %>% 
  mutate(dim = ifelse(dim == 1, "nrGenes", "nrCells")) %>% 
  datatable()  
mypal_1822h <- pal_jama()(2)
```

```{r load-data_1820_mouse}
load("data/unfilteredSeurat_1820_mouse.Rdata")
lapply(list_seurat, function(x) dim(x) %>% enframe(name = "dim")) %>% 
  bind_rows(.id = "experiment") %>% 
  mutate(dim = ifelse(dim == 1, "nrGenes", "nrCells")) %>% 
...
```{r load-data_1822_mouse}
load("data/unfilteredSeurat_1822_mouse.Rdata")
lapply(list_seurat, function(x) dim(x) %>% enframe(name = "dim")) %>% 
  bind_rows(.id = "experiment") %>% 
  mutate(dim = ifelse(dim == 1, "nrGenes", "nrCells")) %>% 
  datatable()  
mypal_1822m <- pal_jama()(2)
```
...
p2 <- VlnPlot(list_seurat_1820[[2]], features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), 
        ncol = 3, pt.size = 0, log = TRUE, combine = FALSE, cols = mypal_1820[2]) 
p2 <- lapply(p2, function(x){
  x + xlab(Project(list_seurat_1820[[2]])) +
    theme(axis.text.x = element_blank(), 
          legend.position = "none")
  })
...
p4 <- VlnPlot(list_seurat_1822[[2]], features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), 
        ncol = 3, pt.size = 0, log = TRUE, combine = FALSE, cols = mypal_1822[2]) 
p4 <- lapply(p4, function(x){
  x + xlab(Project(list_seurat_1822[[2]])) +
    theme(axis.text.x = element_blank(), 
          legend.position = "none")
  })
...
plot2 <- FeatureScatter(list_seurat_1820[[2]], feature1 = "nCount_RNA", feature2 = "percent.mt",
                        cols = mypal_1820[2])
plot2 <- plot2 +
  geom_vline(xintercept = c(nFeature.lower, nFeature.upper)) +
  geom_hline(yintercept = mt.upper)+ 
...
plot3 <- plot3 +
  ...
  scale_x_log10() + 
  theme(legend.position = "none")
plot4 <- FeatureScatter(list_seurat_1822[[2]], feature1 = "nCount_RNA", feature2 = "percent.mt",
                        cols = mypal_1822[2])
plot4 <- plot4 +
  geom_vline(xintercept = c(nFeature.lower, nFeature.upper)) +
  geom_hline(yintercept = mt.upper)+ 
