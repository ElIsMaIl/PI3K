...
include_graphics("plots/fastqc_per_sequence_gc_content_plot_7270.png")
```

```{r fig.cap= "GC Content plot from the 7271 sample."}
include_graphics("plots/fastqc_per_sequence_gc_content_plot_7271.png")
```

```{r fig.cap= "GC Content plot from the 7272 sample."}
include_graphics("plots/fastqc_per_sequence_gc_content_plot_7272.png")
```

```{r fig.cap= "GC Content plot from the 7273 sample."}
include_graphics("plots/fastqc_per_sequence_gc_content_plot_7273.png")
```
...
```{r layout="l-page"}
fname <- "data/7270_Metrics_Summary.csv"
readMetricsSummary(fname) %>% 
  mutate(genome = "human") %>% 
  bind_rows(readMetricsSummary("data/7270_Metrics_Summary_mouse.csv") %>% mutate(genome = "mouse")) %>% 
...
```{r layout="l-page"}
fname <- "data/7271_Metrics_Summary.csv"
readMetricsSummary(fname) %>% 
  mutate(genome = "human") %>% 
  bind_rows(readMetricsSummary("data/7271_Metrics_Summary_mouse.csv") %>% mutate(genome = "mouse")) %>% 
  filter(!is.na(value)) -> metrics
...
```{r layout="l-page"}
fname <- "data/7272_Metrics_Summary.csv"
readMetricsSummary(fname) %>% 
  mutate(genome = "human") %>% 
  filter(!is.na(value)) -> metrics
...
```{r layout="l-page"}
fname <- "data/7273_Metrics_Summary.csv"
readMetricsSummary(fname) %>% 
  mutate(genome = "human") %>% 
  filter(!is.na(value)) -> metrics
...
read_csv("data/7270_UMI_Adjusted_Stats.csv", comment = "##") %>% 
  mutate(genome = "human") %>% 
  bind_rows(
    read_csv("data/7270_UMI_Adjusted_Stats_mouse.csv", comment = "##") %>% 
...
read_csv("data/7271_UMI_Adjusted_Stats.csv", comment = "##") %>% 
  mutate(genome = "human") %>% 
  bind_rows(
    read_csv("data/7271_UMI_Adjusted_Stats_mouse.csv", comment = "##") %>% 
      mutate(genome = "mouse")) -> umi.stats
...
read_csv("data/7272_UMI_Adjusted_Stats.csv", comment = "##") %>% 
  mutate(genome = "human") -> umi.stats
...
read_csv("data/7273_UMI_Adjusted_Stats.csv", comment = "##") %>% 
  mutate(genome = "human") -> umi.stats
...
```{r}
load("data/unfilteredSeurat_7270.Rdata")
load("data/unfilteredSeurat_7270_human.Rdata")
load("data/unfilteredSeurat_7270_mouse.Rdata")
...
```{r}
load("data/unfilteredSeurat_7271.Rdata")
load("data/unfilteredSeurat_7271_human.Rdata")
load("data/unfilteredSeurat_7271_mouse.Rdata")
...
```{r}
load("data/unfilteredSeurat_7272.Rdata")
load("data/unfilteredSeurat_7272_human.Rdata")
load("data/unfilteredSeurat_7272_mouse.Rdata")
...
```{r}
load("data/unfilteredSeurat_7273.Rdata")
load("data/unfilteredSeurat_7273_human.Rdata")
load("data/unfilteredSeurat_7273_mouse.Rdata")
...
```{r}
#| fig.keep = "all",
#| fig.cap = "The first Venn-diagram shows the number of cells from the samples 7270 and their overlap. While the second Venn-Diagram shows the overlap between the human and mouse sample from 7270. The genes which are overlapping are shown in the last output."

#---- map mouse gene names to human gene names
genes_7270m <- convertMouseGeneList(list_seurat_7270m[["7270"]]@assays[["RNA"]]@data)


#---- extract cell list from human and mouse from their Seurat object
cell_List_7270h <- Cells(list_seurat_7270h[["7270"]]@assays[["RNA"]]@data)
cell_List_7270m <- Cells(list_seurat_7270m[["7270"]]@assays[["RNA"]]@data)

#---- extract gene list from human and mouse from their Seurat object
gene_List_7270h <- row.names(list_seurat_7270h[["7270"]]@assays[["RNA"]]@data)


#---- plot Venn diagrams
venn_cells7270 <- venn(list(Human = cell_List_7270h,
                            Mouse = cell_List_7270m))

venn_genes7270 <- venn(list(Human = gene_List_7270h,
                            Mouse = genes_7270m))

#---- show intersection genes between both samples

#isect7270 <- attr(venn_genes7270, "intersections")
#isect7270$"Human:Mouse"

```

```{r}
#| fig.keep = "all",
#| fig.cap = "The first Venn-diagram shows the number of cells from the samples 7271 and their overlap. While the second Venn-Diagram shows the overlap between the human and mouse sample from 7271. The genes which are overlapping are shown in the last output."

#---- map mouse gene names to human gene names
genes_7271m <- convertMouseGeneList(list_seurat_7271m[["7271"]]@assays[["RNA"]]@data@Dimnames[[1]])


#---- extract cell list from human and mouse from their Seurat object
cell_List_7271h <- Cells(list_seurat_7271h[["7271"]]@assays[["RNA"]]@data)
cell_List_7271m <- Cells(list_seurat_7271m[["7271"]]@assays[["RNA"]]@data)

#---- extract gene list from human and mouse from their Seurat object
gene_List_7271h <- row.names(list_seurat_7271h[["7271"]]@assays[["RNA"]]@data)


#---- plot Venn diagrams
venn_cells7271 <- venn(list(Human = cell_List_7271h,
                            Mouse = cell_List_7271m))

venn_genes7271 <- venn(list(Human = gene_List_7271h,
                            Mouse = genes_7271m))

#---- show intersection genes between both samples

#isect7271 <- attr(venn_genes7271, "intersections")
#isect7271$"Human:Mouse"

```

```{r}
#| fig.keep = "all",
#| fig.cap = "The first Venn-diagram shows the number of cells from the samples 7272 and their overlap. While the second Venn-Diagram shows the overlap between the human and mouse sample from 7272. The genes which are overlapping are shown in the last output."

#---- map mouse gene names to human gene names
genes_7272m <- convertMouseGeneList(list_seurat_7272m[["7272"]]@assays[["RNA"]]@data@Dimnames[[1]])


#---- extract cell list from human and mouse from their Seurat object
cell_List_7272h <- Cells(list_seurat_7272h[["7272"]]@assays[["RNA"]]@data)
cell_List_7272m <- Cells(list_seurat_7272m[["7272"]]@assays[["RNA"]]@data)

#---- extract gene list from human and mouse from their Seurat object
gene_List_7272h <- row.names(list_seurat_7272h[["7272"]]@assays[["RNA"]]@data@Dimnames[[1]])


#---- plot Venn diagrams
venn_cells7272 <- venn(list(Human = cell_List_7272h,
                            Mouse = cell_List_7272m))

venn_genes7272 <- venn(list(Human = gene_List_7272h,
                            Mouse = genes_7272m))

#---- show intersection genes between both samples

#isect7272 <- attr(venn_genes7272, "intersections")
#isect7272$"Human:Mouse"

```

```{r}
#| fig.keep = "all",
#| fig.cap = "The first Venn-diagram shows the number of cells from the samples 7273 and their overlap. While the second Venn-Diagram shows the overlap between the human and mouse sample from 7273. The genes which are overlapping are shown in the last output."

#---- map mouse gene names to human gene names
genes_7273m <- convertMouseGeneList(list_seurat_7273m[["7273"]]@assays[["RNA"]]@data@Dimnames[[1]])


#---- extract cell list from human and mouse from their Seurat object
cell_List_7273h <- Cells(list_seurat_7273h[["7273"]]@assays[["RNA"]]@data)
cell_List_7273m <- Cells(list_seurat_7273m[["7273"]]@assays[["RNA"]]@data)

#---- extract gene list from human and mouse from their Seurat object
gene_List_7273h <- row.names(list_seurat_7273h[["7273"]]@assays[["RNA"]]@data)


#---- plot Venn diagrams
venn_cells7273 <- venn(list(Human = cell_List_7273h,
                            Mouse = cell_List_7273m))

venn_genes7273 <- venn(list(Human = gene_List_7273h,
                            Mouse = genes_7273m))

#---- show intersection genes between both samples

#isect7273 <- attr(venn_genes7273, "intersections")
#isect7273$"Human:Mouse"
